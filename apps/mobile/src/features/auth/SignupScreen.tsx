...
export function SignupScreen({ onBack, onSignUp, onLogIn }: SignupScreenProps) {
  ...
  const canSubmit =
    name.trim().length > 0 && email.trim().length > 0 && password.length > 0 && passwordsMatch;
  const showMismatch = password.length > 0 && confirmPassword.length > 0 && !passwordsMatch;
        ...
        >
          ...
          >
            <View style={styles.card}>
              ...
              <Text style={styles.subtitle}>Join in and start building a better city.</Text>
...
const styles = StyleSheet.create({
  ...
  errorText: { fontSize: 12.5, color: "#DC2626", marginTop: 8 },
  submitButton: {
    marginTop: 22,
  ...
  logInText: { fontSize: 13, color: colors.subtext },
  logInLink: { fontSize: 13, color: colors.primaryDark, fontWeight: "700" },
});
